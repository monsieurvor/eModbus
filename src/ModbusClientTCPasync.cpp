...
void ModbusClientTCPasync::onACError(AsyncClient* c, int8_t error) {
  ...
  mb_log_w("TCP error: %s", c->errorToString(error));
}
...
void ModbusClientTCPasync::onPacket(uint8_t* data, size_t length) {
  mb_log_d("packet received (len:%d)", length);
  // reset idle timeout
  MTA_lastActivity = millis();

  if (length) {
    mb_log_d("parsing (len:%d)", length + 1);
  }
  while (length > 0) {
    ...
    if (length > 6) {
      ...
      if (protocolID == 0 &&
        ...
        mb_log_d("packet validated (len:%d)", messageLength);
...
bool ModbusClientTCPasync::send(RequestEntry* re) {
  ...
  if (MTA_client.space() > ((uint32_t)re->msg.size() + 6)) {
    ...
    // done
    MTA_client.send();
    mb_log_d("request sent (msgid:%d)", re->head.transactionID);
    return true;
  }
  return false;
}
